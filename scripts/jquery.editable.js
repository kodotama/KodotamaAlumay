...
(function($){
  ...
  $.fn.editable = function(event, callback){
    ...
    edit.start = function(e){
      ...
      if(dbValue!='')
  	{
    	  old_value = dbValue.replace(/<br( \/)?>/gm, '\n')
    	  					  .replace(/&gt;/gm, '>')
    	  					  .replace(/&lt;/gm, '<')
    	  					  .replace(/^\s+/,'')
    	  					  .replace(/\s+$/,'');
      ...
      input.val(old_value).
        ...
        addClass('editable');
      if(type === 'textarea') input.css('height', '200px');

      var finish = function(){
        var result = input.val().replace(/^\s+/,'').replace(/\s+$/,'');
        var html = escape_html(result);
        html=html.replace(/\[i\s/g,'<i ' );
        html=html.replace(/\[v\s/g,'<v ');
        html=html.replace(/\[c\s/g,'<c ');
        html=html.replace(/]\[\/i]/g,'></i>');
        html=html.replace(/]\[\/v]/g,'></v>');
        html=html.replace(/]\[\/c]/g,'></c>');
        html= html.replace(/\[compa]/g, "<compa>");
        html= html.replace(/\[miss]/g, "<miss>");
        html= html.replace(/\[dict]/g, "<dict>");
        html= html.replace(/\[hash]/g, "<hash>");
        html= html.replace(/\[pblog]/g, "<pblog>");
        html= html.replace(/\[\/compa]/g, "</compa>");
        html= html.replace(/\[\/miss]/g, "</miss>");
        html= html.replace(/\[\/dict]/g, "</dict>");
        html= html.replace(/\[\/hash]/g, "</hash>");
