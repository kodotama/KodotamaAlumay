			...
			<div id="page_content_inner">
				<div class="inner_page">
					<div id="upper_content">
						...
						<p>X7 Chat will now check several configuration properties on your
							server to confirm that the server is compatible with X7 Chat.</p>
							...
							<tbody>
								...
								<?php if($checks_pass): ?>
									...
									<?php if($_SESSION['x7chat_install']['type'] == 'install'): ?>
										<p>X7 Chat has not detected an existing installation and will
							proceed with a new installation.</p>
									<?php else: ?>
										<p>X7 Chat has detected a previous installation and will
							proceed with upgrading the existing installation.</p>
						<p>You should create a backup of your chatroom database before
							upgrading. This installer will not create a backup for you. This
							installer cannot repair, restore or revert your database if the
							upgrade fails.</p>
									<?php endif; ?>
									<a href="index.php?start=1">Click here to begin</a>
								<?php else: ?>
									<p class='fail'>Your current server configuration is not
							compatible with X7 Chat. Please review the items in the table
							with a 'FAIL' result and correct them before installing X7 Chat.</p>
								<?php endif; ?>
						...
						<form action="index.php" method="POST">
							<b><label for="host">Database Host</label></b> <input type="text"
								name="host"
								value="<?php echo isset($_POST['host']) ? sf($_POST['host']) : 'localhost'; ?>" />
							<hr />
							<b><label for="user">Database Username</label></b> <input
								type="text" name="user"
								value="<?php echo isset($_POST['user']) ? sf($_POST['user']) : ''; ?>" />
							<hr />
							<b><label for="pass">Database Password</label></b> <input
								type="password" name="pass"
								value="<?php echo isset($_POST['pass']) ? sf($_POST['pass']) : ''; ?>" />
							<hr />
							<b><label for="dbname">Database Name</label></b> <input
								type="text" name="dbname"
								value="<?php echo isset($_POST['dbname']) ? sf($_POST['dbname']) : ''; ?>" />
							<hr />
							<b><label for="prefix">Table Prefix</label></b> <input
								type="text" name="prefix"
								value="<?php echo isset($_POST['prefix']) ? sf($_POST['prefix']) : 'x7chat_'; ?>" />
							<hr />
						...
						<p>The configuration file, config.php, could not be created
							automatically. Please modify this file and add the following
							content to it:</p>
						...
						<form action="index.php" method="POST">
							<b><label for="admin_username">Admin Username</label></b> <input
								type="text" name="admin_username"
								value="<?php echo isset($_POST['admin_username']) ? sf($_POST['admin_username']) : 'admin'; ?>" />
							<hr />
							<b><label for="admin_username">Admin Password</label></b> <input
								type="password" name="admin_password"
								value="<?php echo isset($_POST['admin_password']) ? sf($_POST['admin_password']) : ''; ?>" />
							<hr />
							<b><label for="retype_admin_password">Retype Admin Password</label></b>
							<input type="password" name="retype_admin_password"
								value="<?php echo isset($_POST['retype_admin_password']) ? sf($_POST['retype_admin_password']) : ''; ?>" />
							<hr />
							<b><label for="admin_email">Admin E-Mail</label></b> <input
								type="text" name="admin_email"
								value="<?php echo isset($_POST['admin_email']) ? sf($_POST['admin_email']) : ''; ?>" />
							<hr />
							<input type="submit" value="Continue" />
						</form>
						...
						<p>
							<a href="../index.php">Visit your chatroom</a>
						</p>
