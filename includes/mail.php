...
class x7_mail {
	...
	public function __construct() {
		...
		if ($this->x7->config ( 'use_smtp' )) {
			...
			$transport = Swift_SmtpTransport::newInstance ( $host, $port, $mode )->setUsername ( $username )->setPassword ( $password );
		} else {
	...
	public function send($to, $template, $vars) {
		...
		$message = Swift_Message::newInstance ( $subject )->setFrom ( $this->x7->config ( 'from_address' ) )->setTo ( $to )->setBody ( $text )->addPart ( $html, 'text/html' );
