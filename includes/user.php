...
class x7_user {
	...
	/**
	 * Returns "Companies'" authority-access types.
	 * 
	 * @author hunoglumurat
	 * @param
	 *        	integer, room_id (companies room.)
	 * @return rooms where this user has access.
	 */
	public function roomAuthorityTypes($room_id) {
		$id = $this->id ();
		...
		$sql = "
			...
			FROM {$this->x7->dbprefix}room_author 
			WHERE
			 user_id = :user_id
			AND  room_id = :room_id
			LIMIT 1;
	 ...
	 * @return rooms where this user has access.
	 */
	public function pageAuthorityTypes($page_id) {
		$id = $this->id ();
		...
		FROM {$this->x7->dbprefix}page_author
		WHERE
		user_id = :user_id
		AND  page_id = :page_id
		LIMIT 1;
