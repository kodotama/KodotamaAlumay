...
class x7chat {
	...
	public function load($lib) {
		$root = realpath ( $_SERVER ["DOCUMENT_ROOT"] );
 		require_once ($root . '/includes/' . $lib . '.php');
	}
	...
	public function urlWithParams($page, $params) {
		...
		if($params){
			...
			foreach ($params  as $singleParam){
				$paramString=$paramString.$singleParam["paramName"]."=".$singleParam["paramValue"];
			}
	...
	public function lang($string, $vars = array()) {
		if (empty ( $this->strings )) {
			...
			$this->strings = require ('./languages/'.$_SESSION["langFile"].'.php');
		}
	...
	public function tl($string) {
		...
		if (empty ( $this->strings )) {
			...
			require ('./languages/'.$_SESSION["langFile"].'.php');
		}
