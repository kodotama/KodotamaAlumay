...
$.fn.storeLocator = function(options) {
  ...
  var settings = $.extend( {
      ...
      'defaultLoc': false,
      'defaultLat': '36.640075',
      'defaultLng': '36.213302',
      'autoGeocode': false,
      'maxDistance': false,
      ...
      'blankInfoWindowTemplatePath': 'templates/blank-info-window.html',
      'addcompanyTemplatePath': 'templates/add-company.html',
...
function initialize(map) {
	...
	google.maps.event.addListener(map, 'mouseover', function(event) {
        ...
        function blankWindow() {
        	...
        	$.get(settings.blankInfoWindowTemplatePath, function(template) {

                	var blankInfoWindowTemplate;
                    var source = template;
                    var blankInfoWindowJSON={  "infoText":info_setting_comp };
            	  
                    blankInfoWindowTemplate = Handlebars.compile(source);
            	    strContent= blankInfoWindowTemplate(blankInfoWindowJSON);
            	    blankInfoWindowVar = new google.maps.InfoWindow({
                        content: strContent
                    });

            
            	    
            	    blankInfoWindowVar.open(map,marker);
        ...
        function placeMarker() {
        	...
        	$.get(settings.addcompanyTemplatePath, function(template) {

                	var addCompanyTemplate;
                	var strContent= "";
                	 

                  getAddress(marker.position);
            	  var source = template;
            	  var addComJSON={  "address":strAddress,
            			  			"lat":marker.position.lat(),
            			  			"lng":marker.position.lng(),
            	    				"placeHolderCompany":placeHolderCompany, 
            						"setUpCompanyButtonLabel" : setUpCompanyButtonLabel,
            						"webTemplate":webTemplate,
            						"telNoTemplate":telNoTemplate
            						
            	  				};
            	    addCompanyTemplate = Handlebars.compile(source);
            	   
            	     
                	
               	    strContent= addCompanyTemplate(addComJSON);
            	     infoWindowVar = new google.maps.InfoWindow({
                        content: strContent
                    });

            
            	    
            	     infoWindowVar.open(map,marker);
